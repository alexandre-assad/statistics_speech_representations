...
class PraatAcousticExtractor(AcousticExtractorService):
    def extract(self, token: PhonemeToken, audio_path: str) -> AcousticFeatures:
        ...
        f3 = formants.get_value_at_time(3, rel_mid) if any(v in token.label for v in "aeiouyøœ") else None
        ...
        if duration > 0.08:
            ...
            f1_traj = {"25%": formants.get_value_at_time(1, t25), "75%": formants.get_value_at_time(1, t75)}
            f2_traj = {"25%": formants.get_value_at_time(2, t25), "75%": formants.get_value_at_time(2, t75)}

        return AcousticFeatures(
            f1=f1, f2=f2, f3=f3, f0=f0,
            f1_trajectory=f1_traj, f2_trajectory=f2_traj
        )