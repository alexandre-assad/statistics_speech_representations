...
def run_parsing(corpus_dir: str, metadata_path: str, output_path: str, occurrence_path: str):
    ...
    for tg_file in Path(corpus_dir).rglob("*.TextGrid"):
        speaker_id = next((s for s in valid_speakers if s.lower() in tg_file.name), None)
        if not speaker_id: 
            print('No speaker id')
            continue
